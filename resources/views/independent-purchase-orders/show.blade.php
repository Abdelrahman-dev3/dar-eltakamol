...
<div class="st-page">
    <section class="st-hero">
        <div>
            ...
            <p class="st-hero-subtitle">{{ __('إدارة حالة الطلب ومراجعة عروض البيع المرتبطة به.') }}</p>
        </div>
    ...
    <section class="st-card">
        <div class="st-info-grid">
            ...
            <div class="st-info-item"><span class="st-info-label">{{ __('تاريخ الطلب') }}</span><div class="st-info-value">{{ $order->requested_at?->format('Y-m-d H:i') }}</div></div>
            <div class="st-info-item"><span class="st-info-label">{{ __('تاريخ النشر') }}</span><div class="st-info-value">{{ $order->published_at?->format('Y-m-d H:i') ?: '-' }}</div></div>
            ...
            <div class="st-info-item"><span class="st-info-label">{{ __('المقبول') }}</span><div class="st-info-value">{{ number_format((float) $order->accepted_shares, 2) }}</div></div>
            <div class="st-info-item st-info-wide"><span class="st-info-label">{{ __('الوصف') }}</span><div class="st-info-value">{{ $order->notes ?: '-' }}</div></div>
        </div>
    </section>
            ...
            <div class="st-form-grid">
                <div class="st-field">
                    ...
                    <select class="st-select" name="status" required>
                        <option value="{{ \App\Models\IndependentPurchaseOrder::STATUS_PENDING }}" @selected((int) $order->status === \App\Models\IndependentPurchaseOrder::STATUS_PENDING)>{{ __('قيد الانتظار') }}</option>
                        <option value="{{ \App\Models\IndependentPurchaseOrder::STATUS_PUBLISHED }}" @selected((int) $order->status === \App\Models\IndependentPurchaseOrder::STATUS_PUBLISHED)>{{ __('منشور') }}</option>
                        ...
                        <option value="{{ \App\Models\IndependentPurchaseOrder::STATUS_CLOSED }}" @selected((int) $order->status === \App\Models\IndependentPurchaseOrder::STATUS_CLOSED)>{{ __('مغلق') }}</option>
                    </select>
    ...
    <section class="st-card">
        <div class="st-card-header"><div><h2 class="st-card-title">{{ __('عروض البيع المقدمة') }}</h2></div></div>
        <div class="st-table-wrap">
            <table class="st-table">
                <thead><tr><th>#</th><th>{{ __('المساهم') }}</th><th>{{ __('الكمية') }}</th><th>{{ __('السعر') }}</th><th>{{ __('المقبول') }}</th><th>{{ __('الحالة') }}</th></tr></thead>
                <tbody>
                @forelse($order->sellOffers as $offer)
                    <tr>
                        ...
                        <td>{{ $offer->getIndependentOfferStatusText() }}</td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="st-empty">{{ __('لا توجد عروض بيع مرتبطة بهذا الطلب.') }}</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
