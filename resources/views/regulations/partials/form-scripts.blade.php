...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const multipleFilesInput = document.querySelector('[data-reg-files]');
        ...
        function renderFiles(files, container) {
            ...
            Array.from(files || []).forEach(function (file, index) {
                ...
                item.className = 'reg-file-item';
                item.innerHTML =
                    '<div class="reg-file-meta">' +
                        '<span class="reg-file-icon"><i class="bi ' + fileIcon(file.name) + '"></i></span>' +
                        '<div>' +
        ...
        form.addEventListener('submit', function (event) {
            ...
            if ((form.dataset.requireName || '') === 'true' && !name) {
                alert(form.dataset.emptyName || 'يرجى إدخال اسم اللائحة');
                event.preventDefault();
                return;
            }
